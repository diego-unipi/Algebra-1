...
\begin{lemma}[Lemma di Ranieri]
    \label{davide}
    Sia $G$ un gruppo abeliano di ordine $n$, allora $\forall d\mid n, \exists H \leqslant G : |H| = d$.\footnote{La dimostrazione non è stata fatta durante il corso, ma è stata comunque aggiunta per completezza.}
\end{lemma}
    ...
    Dimostriamo tutte le affermazioni del teorema:
        \begin{enumerate}[(1)]
            ...
            \item Sia $S$ un $p$-sottogruppo di Sylow di $G$, con $|S| = p^n$, e sia $H \leqslant G$, con $|H| = p^{\alpha}$; consideriamo l'insieme $\faktor{G}{S} = X$ dato dalle classi laterali di $S$ in $G$, allora:
                ...
                \[ \varphi_h(g_iS) = hg_iS = g_iS \iff hg_i \in g_iS \iff h \in g_iSg_{i}^{-1}\iff H \subseteq g_iSg_i^{-1}
                    \]
                ...
                Per completare la dimostrazione del punto $(2)$ utilizziamo il risultato del \hyperref[l:1.95]{Lemma 1.95}, considerando $|H| = p^{\alpha}$, con $\alpha \leq n - 1$ e $H \lneq S$ ($S$ è un $p$-Sylow), abbiamo $H \lneq N_S(H)$ \footnote{Si noti che abbiamo preso il normalizzatore di $H$ in $S$.},
                sia ora $\frac{N_{S}(H)}{H}$, esso è un $p$-gruppo non banale e per il \hyperref[p:Cauchy]{Teorema di Cauchy} esiste una classe laterale $\overline x (= xH)$ di ordine $p$, inoltre per il Teorema di Corrispondenza \footnote{Tra i sottogruppi di $\displaystyle\frac{N_{S}(H)}{H}$ ed i
            ...
            dunque per quanto sappiamo sul numero di coniugi rispetto all'azione di coniugio si ha $n_p = |\mathcal{C}\ell(S)|=[G:N_G(S)]$\footnote{Dove $S$ è un fissato $p$-Sylow.}, da cui:
...
\begin{example}[Ideali]
    ...
    \begin{itemize}
        ...
        \item I sottogruppi $\{0\}$ (ideale \vocab{banale}) e $A$ (ideale \vocab{improprio}) sono ideali di $A$.
    \end{itemize}
\end{example}
...
\begin{example}
    ...
    \begin{itemize}
        ...
        \item Sia $A$ un dominio e $P \subset A$ un suo ideale primo, possiamo considerare $S = A \setminus P$ che è una parte moltiplicativa, in quanto 
            ...
            sta ancora nel suo complementare $xy \in A \setminus P = S$. In questo caso indichiamo $S^{-1}A = A_p$ e prende il nome di \vocab{localizzato} dell'anello $A$ all'ideale $P$.
    \end{itemize}
\end{example}
...
\begin{proof}
    ...
    \begin{itemize}
        \item Se $c(f(x)) = c(g(x)) = 1$, dunque $f(x)$ e $g(x)$ sono primitivi, vogliamo verificare che $c(f(x)g(x)) = 1$; se $c(f(x)g(x))$ non fosse primitivo (ovvero associato ad 1)
...
\begin{corollary}[Divisibilità nel campo dei quozienti$\implies$divisibilità]
    \label{2.118}
...
\begin{corollary}[Riducibilità nel campo dei quozienti$\implies$riducibilità]
    \label{2.119}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        per cui $f(x)$ è irriducibile in $A[x]$ se e sole $f(x)$ è irriducibile in $A$ (dato che tra l'altro abbiamo $(A[x])^* = A^*$, ed uno tra $g(x)$ ed $h(x)$ deve essere invertibile).
...
\begin{lemma}[Il campo fissato è quello base$\iff$è fissato rispetto a tutto il gruppo]
    \label{3.66}
...
\begin{lemma}
    [Sottocampo fissato dal coniugato]
    \label{3.67}
