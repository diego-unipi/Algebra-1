...
\begin{example}
    Se $X = \RR^2$ e $G$ è il gruppo di traslazioni di vettore $\ul v = (0, l)$, allora:
        \[ \varphi : G \longrightarrow S(X) : \tau_{(0,l)} \longmapsto \tau_{(0,l)} \footnote{Il primo $\tau_{(0,l)}$ è l'elemento del gruppo (ovvero la traslazione), mentre il secondo è l'applicazione
        bigettiva da $X$ in $X$}
            \]
    con:
