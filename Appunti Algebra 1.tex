...
\begin{proof}
    ...
    \begin{itemize}
        ...
        le loro controimmagini tramite $\pi$ rispettano un contenimento stretto (perché si preservano gli indici). Sempre per corrispondenza $\pi^{-1}\left(N_{G/Z(G)}\left(\faktor{H}{Z(G)}\right)\right)=H$, quindi basta mostrare che $\pi^{-1}\left(N_{G/Z(G)}\left(\faktor{H}{Z(G)}\right)\right)\subseteq N_G(H)$, e questo deriva 
    ...
    Dimostriamo tutte le affermazioni del teorema:
        \begin{enumerate}[(1)]
            ...
            \item Sia $S$ un $p$-sottogruppo di Sylow di $G$, con $|S| = p^n$, e sia $H \leqslant G$, con $|H| = p^{\alpha}$; consideriamo l'insieme $\faktor{G}{S} = X$ dato dalle classi laterali di $S$ in $G$, allora:
                ...
                che impedisce la divisibilità di $m$ per $p$) per questo $i$ si ha che $\Orb(g_iS) = \{g_iS\} \implies \St(g_iS) = H$ (ovvero per tale $i$ si ha una classe
                ...
                sia ora $\frac{N_{S}(H)}{H}$, esso è un $p$-gruppo non banale e per il \hyperref[p:Cauchy]{Teorema di Cauchy} esiste una classe laterale $\overline x (= xH)$ di ordine $p$, infine, per il Teorema di Corrispondenza \footnote{Tra i sottogruppi di $\displaystyle\frac{N_{S}(H)}{H}$ ed i
        ...
        \item Sia $P$ un ideale primo, se fosse $P \cap S \ne \emptyset$, allora per quanto visto al punto (3) non sarebbe proprio (e dunque nemmeno primo), viceversa, se $P \cap S = \emptyset$ vogliamo dimostrare che $S^{-1}P$ primo in $S^{-1}A$; consideriamo:
            ...
            ciò è equivalentemente al fatto che $\exists \sigma \in S$ e $\exists p \in P$ tali per cui:
                \[ \frac{ab}{st} = \frac{p}{\sigma} \iff ab\sigma = \underbrace{p}_{\in P}st \in P \implies ab\sigma \in P
                    \]
            ma, essendo per ipotesi che $\sigma \in P$ e $P \cap S = \emptyset$, allora $ab \in P$, e poiché $P$ è primo si deve avere $a \in P$ o $b \in P$, e quindi la frazione di uno dei due deve essere quella in $S^{-1}P$: $\displaystyle\frac{a}{s} \in S^{-1}P \qquad \text o \qquad \frac{b}{t} \in S^{-1}P$
...
\begin{proposition}[Numero di estensioni  via identità di $K(\alpha)$ a $\overline K$]
    ...
    \[ \exists \varphi_1,\ldots,\varphi_k : K(\alpha) \varlonghookrightarrow \overline K \qquad \text{con} \qquad \varphi_{i|K} = id_K
        \]
    ovvero esistono esattamente $k$ estensioni distinte da $K(\alpha)$ a $\ol K$, che estendono l'immersione di $K$ in $\ol K$ per mezzo dell'identità.
\end{proposition}
...
\begin{proof}
    ...
    \[ \widetilde{\varphi} : K[x] \longrightarrow \overline K : x \longmapsto \beta , p(x) \longmapsto p(\beta) \qquad \forall \beta \in \overline K
        \]
    per tale omomorfismo abbiamo che $\displaystyle K(\alpha) \cong \frac{K[x]}{(\mu_\alpha(x))}$ se e solo se $(\mu_\alpha(x)) \subset \ker \widetilde{\varphi} \iff \mu_\alpha(x) \in \ker \widetilde{\varphi}$ (per le proprietà degli ideali), quindi se e solo se $\mu_\alpha(\beta) = 0$,
...
\begin{definition}
    Un campo $K$ tale per cui tutti i polinomi irriducibili in $K[x]$ hanno derivata non nulla prende il nome di \vocab{campo perfetto}.\footnote{Dunque per il criterio precedente hanno radici tutte distinte.}
\end{definition}
...
\begin{proof}
    ...
    \[ \widetilde{\varphi} : K[x] \longrightarrow \overline K : x \longmapsto \beta : x \longmapsto \beta, p(x) \longmapsto (\varphi(p(x)))(\beta)\footnote{Cioè applichiamo $\varphi$ a $p(x)$ e poi valutiamo il nuovo polinomio ottenuto in $\beta$.} \qquad \text{con} \qquad \varphi_{i|K} = \varphi
        \]
...
\begin{example}[Immersioni nella chiusura algebrica e coniugati]
...
\begin{example}[Polinomio ciclotomico $p$-esimo]
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        in tanti modi quanti il grado $[F : K(\alpha_1)]$ (\hyperref[3.39]{Proposizione 3.39}). Fissata un'estensione di $\varphi_i$:
        ...
        dalla radice $\alpha_1$ di $f(x)$, che va nei suoi coniugati mediante $\varphi_i$, otteniamo tutte le altre radici dentro $F$\footnote{Il punto di questa dimostrazione è che se aggiungiamo una radice all'estensione, allora esistono degli omomorfismi, grazie all'ipotesi di normalità, che ci permettono di ottenerle tutte.}:
        ...
        \end{multline*}
        dove il contenimento segue dal fatto che abbiamo soltanto permutato gli elementi $\{\alpha_{ij}\}$, per cui $K \subset \varphi(F) \subseteq F$, ma poiché $F$ e $\varphi(F)$ hanno lo stesso grado finito su $K$,
        allora $\varphi(F) = F$.
    \end{itemize}
\end{proof}
...
\begin{proposition}
[Proprietà delle estensioni normali rispetto al composto ed all'intersezione di estensioni]
...
\[ \Aut_K E = \{\varphi : E \underset{\sim}{\longrightarrow} E| \varphi_{|K} = id_K\}
    \]
i $K$-automorfismi di $E$, cioè gli automorfismi dell'estensione che fissano puntualmente $K$, tale insieme con l'operazione di composizione forma il \vocab{gruppo di Galois}:
...
\begin{proposition}[Grado del campo di spezzamento]
    \label{3.55}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \[ \phi(\varphi \circ \psi) = (\varphi \circ \psi)_{|{\{\alpha_1,\ldots,\alpha_n\}}} = \varphi(\psi_{|{\{\alpha_1,\ldots,\alpha_n\}}}) = \varphi_{|{\{\alpha_1,\ldots,\alpha_n\}}} \circ \psi_{|{\{\alpha_1,\ldots,\alpha_n\}}} = \phi(\varphi) \circ \phi(\psi)
            \]
        $\forall \phi,\psi \in \Gal\left(\faktor{F}{K}\right)$, in questo caso stiamo restringendo $\varphi$ alle radici perché $\psi$ sulle radici ha immagine nell'insieme stesso, per quanto detto sopra.    
...
\[ \Orb(\alpha_1) = \left\{\varphi(\alpha_1) \m \varphi \in \Gal\left(\faktor{F}{K}\right)\right\} = \{\alpha_1,\ldots,\alpha_n\}
    \]
infatti, essendo $[K(\alpha_1) : K] = n$, allora abbiamo esattamente $n$ immersioni che permutano i coniugati (ognuna delle quali si può estendere ad $F$):
...
Con $\ord \varphi = d$ in quanto $\phi^d(x) = \underbrace{\phi \circ \ldots \phi}_{\text{$d$ volte}}(x) = x^{q^d} = x$.
