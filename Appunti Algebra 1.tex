...
\begin{proposition}[Grado di un'estensione]
    \label{3.7}
    Data l'estensione $\faktor{L}{K}$ e $\alpha \in L$, allora:
...
\begin{proposition}
    ...
    \label{torri}
    Data una torre di estensioni $K \subset F \subset L$, $\faktor{F}{K}$ è finita se e solo se $\faktor{F}{L}$ e $\faktor{L}{K}$ sono finite, inoltre:
    \[ [F:K] = [F:L][L:K]
        \]
\end{proposition}
...
\begin{proof}
    Vogliamo verificare che $\forall \alpha \in L$, $\alpha$ è algebrico su $K$, abbiamo che:
    \[ K \subseteq K(\alpha) \subseteq L
        \]
    è finita per ipotesi, pertanto la sottoestensione $\faktor{K(\alpha)}{K}$ è a sua volta finita (volendo per il \hyperref[torri]{Teorema delle torri}),
    quindi per la \hyperref[3.7]{Proposizione 3.7} $\alpha$ è algebrico su $K$, ed essendo vero per ogni $\alpha$, allora $L$ è un'estensione algebrica di $K$.
\end{proof}
...
\begin{proof}
    ...
    \[ K \subseteq K(\alpha) \subseteq K(\alpha)(\beta) = K(\alpha,\beta)
        \]
    la prima estensione è finita per ipotesi, mentre la seconda è finita in quanto estensione finita di una sottoestensione (essendo $\beta$ algebrico
    su $K$, lo sarà anche su $K(\alpha)$, che è un insieme più grande). Essendo $K \subseteq K(\alpha,\beta)$ un'estensione finita, per la \hyperref[3.11]{Proposizione 3.11}
...
\begin{remark}
    Il viceversa della \hyperref[3.11]{Proposizione 3.11} è falso.
\end{remark}
...
\begin{example}
    ...
    è un'estensione algebrica di $\QQ$. Verifichiamo che $[\overline\QQ:\QQ] = +\infty$, infatti, considerata la torre:
    ...
    \[ [\overline \QQ: \QQ] = [\overline \QQ: \QQ(\sqrt[n]{2})] [\QQ(\sqrt[n]{2}) : \QQ] \geq n \qquad \forall n \geq 2
        \]
    dunque l'estensione $\faktor{\overline\QQ}{\QQ}$ ha grado $+\infty$.
\end{example}
...
\begin{proof}
    Possiamo dimostrare la tesi per induzione; per $n = 1$, la tesi è vera per la \hyperref[3.11]{Proposizione 3.11}. Supponiamo la tesi vera per $n-1$, abbiamo che:
    ...
    \[ K \subset K(\alpha_1,\ldots,\alpha_{n-1}) \subset K(\alpha_1,\ldots,\alpha_{n-1})(\alpha_n)
        \]
    dove la prima estensione è finita per ipotesi induttiva, mentre la seconda perché estensione semplice, dunque vale ancora la \hyperref[3.11]{Proposizione 3.11}, pertanto 
    $L$ è finita.
\end{proof}
...
Dalla proposizione appena dimostrata ci poniamo un secondo problema, quello di contare il numero di radici di $\mu_\alpha (x)$ in $\ol K$, essendo $\ol K$ algebricamente chiuso, allora il numero delle radici del polinomio, ciascuna contata 
...
\begin{remark}
    ...
    \begin{itemize}
        ...
        Inoltre se $\alpha \in \ol K$ è una radice di $f(x)$, abbiamo che $f(\alpha) = \alpha^p - t = \implies t = \alpha^p$, da cui:
        \[ f(x) = x^p - \alpha^p = (x - \alpha)^p
            \]
        dove nell'ultima uguaglianza abbiamo usato il lemma del Binomio Ingenuo, pertanto in realtà $f(x)$ ha un'unica radice in $\ol K$.
    \end{itemize}
\end{remark}

Ci limiteremo (assumendolo anche senza ripeterlo ogni volta) ai campi perfetti, pertanto un polinomio irriducibile di $K[x]$ di grado $n$ avrà esattamente $n$ radici distinte in $\ol K$.

\begin{proposition}[Numero di estensioni di $K(\alpha)$ a $\overline K$]
    \label{3.37}
    Dato $\alpha \in \ol K$, con $[K(\alpha) : K] = n$, si ha che $\forall \varphi : K \varlonghookrightarrow \ol K$ esistono esattamente $n$ estensioni a $K(\alpha)$, cioè:
    \[ \exists \varphi_1,\ldots,\varphi_n : K(\alpha) \varlonghookrightarrow \ol K \qquad \text{con} \qquad \varphi_{i|K} = \varphi
        \]
\end{proposition}

\begin{remark}
    Abbiamo già visto che ciò è vero se $\varphi = id_K$, nella \hyperref[3.33]{Proposizione 3.33},
    la nuova proposizione ci permette di contare il numero di estensioni da $K(\alpha)$ (estensione semplice) a $\ol K$ con un qualsiasi omomorfismo, ad esempio per $K = \QQ(\sqrt[3]{2})$ abbiamo:
...
\begin{proof}
    ...
    \[ \widetilde{\varphi} : K[x] \longrightarrow \ol K : x \longmapsto \beta : p(x) \longmapsto p(\beta) \qquad \text{con} \qquad \varphi_{i|K} = \varphi
        \]
    ...
    è una radice di $\varphi(\mu_\alpha(x))$, pertanto le estensioni di $\varphi$ a $K(\alpha)$ sono tante quante le radici distinte di $\varphi(\mu_\alpha(x))$ in $\ol K$. Poiché $\mu_\alpha(x)$ è irriducibile per definizione, allora
...
\begin{corollary}[Numero di estensioni a $\ol K$ di un'estensione qualsiasi]
    \label{3.39}
    Sia $\faktor{E}{K}$ un'estensione, con $[E:K] = n$, allora $\forall \varphi : K \varlonghookrightarrow \ol K$ immersione, esistono $n$ immersioni:
    \[ \varphi_1,\ldots,\varphi_n : E \varlonghookrightarrow \ol K \qquad \text{con} \qquad \varphi_{i|K} = \varphi
        \]
\end{corollary}
...
\begin{proof}
    ...
    $\varphi$ si estende:
    \[ \varphi_1,\ldots,\varphi_m : K(\alpha) \varlonghookrightarrow \ol K
        \]
    Ogni $\varphi_i : K(\alpha) \varlonghookrightarrow \ol K$ si estende a sua volta per ipotesi induttiva:
    \[ \varphi_{i1},\ldots,\varphi_{id} : E \varlonghookrightarrow \ol K
        \]
    dunque abbiamo $\{\varphi_{ij}\}_{i = 1,\ldots,m}^{j = 1,\ldots,d} : E \varlonghookrightarrow \ol K$, ma $\varphi_{ij | K(\alpha)} = \varphi_i $, dunque $\varphi_{ij|K} = \varphi_{i | K} = \varphi$, quindi abbiamo trovato le $n$ estensioni.
\end{proof}
...
\begin{definition}
    Dato $\alpha \in \ol K$, diciamo che i \vocab{coniugati} di $\alpha$ su $K$ sono le radici del polinomio minimo di $\alpha$ su $K$. 
\end{definition}
...
\begin{example}
    Sia $f(x) = x^3 - 2$, tale polinomio coincide con $\mu_{\sqrt[3]{2}/\QQ}(x)$, ovvero il polinomio minimo di $\alpha = \sqrt[3]{2}$ su $\QQ$, vogliamo studiare le immersioni di $\QQ(\alpha)$ in $\ol \QQ$:
    \[ \varphi : \QQ(\alpha) \varlonghookrightarrow \ol \QQ \qquad \text{con} \qquad \varphi_{i|\QQ} = id_\QQ
        \]
...
\begin{example}
    ...
    \[ \varphi_i : \QQ(\zeta_p) \longrightarrow \ol \QQ : \zeta_p \longmapsto \zeta_p^i \qquad \text{con} \qquad \varphi_{i|\QQ} = id_\QQ
        \]
...
\begin{definition}
    ...
    \[ \forall \varphi : F \varlonghookrightarrow \ol K \qquad \text{con} \qquad \varphi_{|F} = id_F
        \]
...
\begin{example}
    ...
    \begin{itemize}
        ...
        \[ \varphi : F \varlonghookrightarrow \ol \QQ \qquad \text{con} \qquad \varphi_{|F} = id_F
            \]
        con:
...
\begin{proof}
    ...
    \begin{itemize}
        \item \underline{\textbf{$(1)\implies (2)$}}: sia $f(x) \in K[x]$ e siano $\alpha_1,\ldots,\alpha_n \in \ol K$ le radici di $F$, per ipotesi sappiamo che $f(x)$ ha almeno una radice in $F$, supponiamo sia $\alpha_1$, allora $K(\alpha_1) \subset F$, dunque
        $\forall i \in \{1,\ldots,n\}$ consideriamo le immersioni:
        \[ \varphi_i : K(\alpha_1) \varlonghookrightarrow K(\alpha_i) \subseteq \ol K : \alpha_1 \longmapsto \alpha_i \qquad \text{con} \qquad \varphi_{i|K} = id_K
            \]
        ...
        \[ \widetilde{\varphi_i} : (K(\alpha_i) \subset)F \longrightarrow \ol K \qquad \text{con} \qquad \widetilde{\varphi}_{i|K(\alpha_1)} = \varphi_i \implies \widetilde{\varphi}_{i|K} = id_K
            \]
        ...
        \[ \varphi : F \varlonghookrightarrow \ol K \qquad \text{con} \qquad \varphi_{|K} = id_K
            \]
...
Date due estensioni $\faktor{F}{K}$ e $\faktor{L}{K}$ in una fissata chiusura algebrica $\ol K$ normali, allora $\faktor{FL}{K}$ e $\faktor{F \cap L}{K}$ sono normali.
\end{proposition}

\begin{proof}
    La proprietà del composto segue immediatamente dal fatto che l'immersione nella chiusura algebrica:
    \[ \varphi : FL \longrightarrow \ol K \qquad \text{con} \qquad \varphi_{|FL} = id_K
        \]
...
\label{3.50}
Data una torre di estensioni $K \subset F \subset L$ in una fissata chiusura algebrica $\ol K$, se $\faktor{L}{K}$ è normale, allora $\faktor{L}{F}$ è normale.\footnote{In generale $\faktor{F}{K}$ non è normale.}
\end{proposition}
...
\[ \{\varphi : E \varlonghookrightarrow \ol K| \varphi_{|K} = id_K\}
    \]
...
\begin{proof}
    Dette $\alpha_1,\ldots,\alpha_n$ le radici di $f(x)$ in $\ol K$, allora $F = K(\alpha_1,\ldots,\alpha_n)$, da cui si ha la torre:
...
\[ \forall i \in \{1,\ldots,n\},  \exists \psi_i : K(\alpha_1) \longrightarrow K(\alpha_i) \subset \ol K : \alpha_1 \longmapsto \alpha_1
    \]
...
\[ \{\psi(\alpha)\}
    \]
che, al variare delle immersioni $\psi$ di $\QQ(\alpha)$ in $\ol \QQ$, è l'insieme delle radici di $\mu_\alpha(x)$, dunque possiamo scrivere:
...
\begin{proof}
    Osserviamo che:
    \[ \forall \varphi : \Fpn \varlonghookrightarrow \ol \Fp
        \]
    $\varphi(\Fpn)$ è un sottocampo di $\ol\Fp$ con $p^n$ elementi, e per l'unicità dei campi con $p^n$ elementi deve essere necessariamente che $\varphi(\Fpn) = \Fpn$,
