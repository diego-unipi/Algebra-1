...
\begin{soln}
    ...
    per il \hyperref[davide]{Lemma Di Ranieri}\footnote{Il nome ovviamente non è ufficiale, ma deriva da un curioso aneddoto in cui è coinvolto il buon Davide
    Ranieri, pertanto vi sconsiglio di citarlo con questo nome in contesti ufficiali, ma se voleste farlo comunque è a vostro rischio e pericolo :)}) sono normali in $Z(G)$, inoltre $|Z(G)| = p^z$ (dunque si hanno sottogruppi normali di ordine $p^l$ per $l \mid z$), pertanto esiste la catena:
        \[ \{e\} = H_n < \ldots < H_m = Z(G)
...
\begin{remark}
    ...
    che $(-1)a \in I$, dove $(-1)$ esiste in $A$ poiché è unitario. \footnote{Da questo momento in poi, anche se non specificato assumiamo di star operando sempre in un anello commutativo con identità.}
\end{remark}
...
\begin{example}[Ideali]
    ...
    \begin{itemize}
        ...
        \item I sottogruppi $\{0\}$ (ideale \vocab{banale}), $A$ (ideale \vocab{improprio}) sono ideali dell'anello $A$.
    \end{itemize}
\end{example}
...
\begin{proof}
    ...
    poiché stiamo operando nel caso di anelli commutativi). Presi $x,y \in S$, ovvero della forma:
        \[ x = \sum_{i = 1}^n a_is_i \qquad \text e \qquad y = \sum_{j=1}^m\alpha_j \sigma_j \qquad a_i,\alpha_j \in A \qquad s_i,\sigma_j \in S 
            \]
    si osserva che:
        \[ x + y = \sum_{i = 1}^n a_is_i + y = \sum_{j=1}^m\alpha_j \sigma_j \in (S)
            \]
    duqnue $I$ è chiuso per la somma. Infine, per ogni $a \in A$ si ha:
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item Dato $I + J = \{i + j | i \in I, j \in J\}$, presi $x,y \in I + J$, ovvero:
            ...
            \[ i+j \in (I,J) \implies I + J \subseteq (I,J)
                \]
            ...
            $I + J$ contiene il più piccolo ideale di $A$ che contiene sia $I$ che $J$. Osserviamo che:
                \[ (S) = \bigcap_{\substack{S \subseteq X \subseteq A\\ X\,\text{ideale}}} X
                    \]
            dove l'intersezione è il più piccolo ideale di $A$ che contiene $S$. Dimostriamo ora l'uguaglianza, $(S)$ è contenuto nell'intersezione in quanto
            è uno dei termini di quest'ultima; il contenimento opposto segue dal fatto che $\forall x \in S$ si ha $x = \sum a_is_i \in X$ (poiché $X$ è un ideale che contiene $S$),
            pertanto, per vedere che $(S)$ è contenuto in $\mathcal{I}$, ideale di $A$, basta vedere che $S \subset \mathcal{I}$ (e ciò appunto giustifica il fatto che basti mostrare che $I,J$ sono 
            contenuti in $I+J$). A questo punto si osserva che, essendo $0 \in J$ abbiamo:
                \[ I \subset I + J
                    \]
            infatti basta prendere sempre $j = 0$ per ottenere tutti gli elementi di $I$; ugualmente si dimostra la stessa cosa per $J$, dunque $I,J \subset I + J \implies (I,J) \subset I + J \implies I + J = (I,J)$.
        ...
        \item Verifichiamo che $\sqrt{I} = \{x \in A | x^n \in I, n \in \NN\}$ è un ideale, presi $x,y \in \sqrt{I}$, ovvero $x^n,y^m \in I$, $n,m \in \NN$, vogliamo 
            provare che $x + y \in \sqrt{I}$ (ovvero che esiste $d \in \NN$ tale che $(x+y)^d \in I$), osserviamo che:
                \[ (x + y)^{n + m} = \sum_{i = 0}^{n + m} \binom{n+m}{i} x^i y^{m+n-i}
                    \]
            dove $\forall i \{0,\ldots,m+n\}$ si ha che $i \geq n \implies x^i \in I$, oppure che $n+m-i \geq m \implies y^{n+m-i} \in I$, dunque tutti i termini di $(x+y)^{n+m}$ stanno in $I$
...
\begin{remark}
    Osserviamo che $IJ \subset I \cap J$, poiché $I \cap J$ è ideale basta vedere che contiene un insieme di generatore di $IJ$. Ma questo è ovvio perché, presi $x \in I$ e
    $y \in J$ si ha:
...
\begin{remark}
    Se $I + J = A$, allora $IJ = I \cap J$, infatti, per ipotesi sappiamo che:
        \[ i+j = 1
            \]
    occorre dunque vedere che $\forall x \in I \cap J$ (per l'osservazione precedente sapevamo già che $IJ \subset I \cap J$, quindi stiamo verificando il contenimento opposto),
...
\begin{proof}
    ...
    \begin{enumerate}[(1)]
        ...
        \item $A$ è un campo se e solo se $A^* = A \setminus \{0\}$, ma per il punto (1) l'unico elemento fuori da $A^*$ è 0, e $I = \{0\}$ è proprio l'unico ideale proprio che possiamo avere.
    \end{enumerate}
\end{proof}
