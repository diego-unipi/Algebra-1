...
\begin{example}
    ...
    \[ |HH^{\prime}| = \frac{|H||H^{\prime}|}{|H \cap H^{\prime}|} = \frac{q \cdot q}{1} = q^2 > pq
        \]
    quindi $H^{\prime}$ non può essere un sottogruppo di $G$. Si verifica che, detto $K = \left<y\right>$, le ipotesi del \hyperref[t:1.72]{Teorema 1.72} sono soddisfatte:
...
\begin{remark}
    ...
    Sia:
        \[ G(p) = \{g \in G | \ord(g) = p^k, k \in \NN\}
            \]
    $G(p)$ prende il nome di \vocab{$p$-sottogruppo di Sylow} di $G$, o anche \vocab{$p$-componente} o componente di \vocab{$p$-torsione}.
    Si osserva che:
        \begin{itemize}
...
\begin{theorem}
    [I $p$-gruppi si spezzano come prodotti di $p$-gruppi ciclci]
    \label{t:t2}
...
Segue la dimostrazione del \hyperref[t:struttra]{Teorema Di Struttura Dei Gruppi Abeliani Finiti}:
...
\begin{example}
    ...
    otteniamo i $p$-sottogruppi:
        \[ G \cong \underbrace{(\Z{2^3} \times \Z{2^2} \times \Z{2})}_{G(2)} \times \underbrace{(\Z3 \times \Z3)}_{G(3)} \times \underbrace{(\Z{5^2} \times \Z5)}_{G(5)}
            \]
    e per il \hyperref[t:struttra]{Teorema Di Struttura} possiamo riscrivere il prodotto in ordine decrescente (rimettendo assieme $p$-gruppi cicli di ordine massimo):
...
\begin{example}
    ...
    \end{cases}
        \]
    Dunque i gruppi abeliani di ordine $1000$ (a meno di isomorfismo) sono $3\cdot 3 = 9$, in quant per il \hyperref[t:struttra]{Teorema di Struttura} abbiamo
    ...
    \begin{itemize}
        ...
        \item Se $r_1 < n$, poiché $G$ è abeliano si ha $\left<x_1\right> \triangleleft G$, quindi possiamo considerare $\faktor{G}{\left<x_1\right>}$ che ha ordine $p^{n-r_1} < p^n$, dunque vale l'ipotesi induttiva ed il gruppo quoziente può essere fattorizzato come prodotto di gruppi ciclici:
            ...
            Per mostrare che $\left<x_1\right> + H = G$, osserviamo che $\left<x_1\right> + H = G \subseteq G$ e che la sua cardinalità è:
                \[ |\left<x_1\right> + H| = \frac{|\left<x_1\right> + H = G||H|}{|\left<x_1\right> \cap H|} = \frac{p^{r_1}\cdot p^{n - r_1}}{1} = p^n
                    \]
