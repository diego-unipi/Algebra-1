...
\begin{remark}
    ...
    che $(-1)a \in I$, dove $(-1)$ esiste in $A$ è un gruppo rispetto al $+$.\footnote{Da questo momento in poi, anche se non specificato assumiamo di star operando sempre in un anello commutativo con identità.}
\end{remark}
...
\begin{definition}
    Dato un campo $K$ ed una sua estensione $L$, $\alpha \in L$ si dice \vocab{trascendete} su $K$ se non è algebrico, ovvero:
...
\begin{remark}
	...
	\begin{itemize}
	\ii $\alpha$ è trascendete su $K$ $\iff \ker{\varphi_\alpha} = \{0\} \iff \varphi_\alpha$ è iniettivo $\iff K[x] \cong K[\alpha]$.
...
\begin{proposition}[Estensione alegberica e finitamente generata$\implies$finita]
    \label{3.16}
    Sia $\faktor{L}{K}$ un'estensione algebrica e finitamente generata, $L = K(\alpha_1,\ldots,\alpha_n)$, allora $\faktor{L}{K}$ è finita.
\end{proposition}
