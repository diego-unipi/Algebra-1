...
Leonardo Alfani, Clementina Salamina, Giorgia Capecchi, Gianni Bellu, Lorenzo Picinelli.
...
\begin{remark}
    Osserviamo che $IJ \subset I \cap J$, poiché $I \cap J$ è ideale basta vedere che contiene un insieme di generatori di $IJ$. Ma questo è ovvio perché, presi $x \in I$ e
    $y \in J$ si ha dalla proprietà di assorbimento che:
        \[ \underbrace{x}_{\in I}\underbrace{y}_{\in A} \in I \qquad \text e \qquad \underbrace{x}_{\in A}\underbrace{A}_{\in J} \in  J \implies xy \in I \cap J
            \]
\end{remark}

\begin{remark}
    \label{2.24}
    Se $I + J = A$, allora $IJ = I \cap J$, infatti, per ipotesi sappiamo che:
        \[ i+j = 1
            \]
    occorre dunque vedere che $\forall x \in I \cap J$ si ha $x \in IJ$(per l'osservazione precedente sapevamo già che $IJ \subset I \cap J$, quindi stiamo verificando il contenimento opposto),
...
\begin{theorem}
    ...
    Sia $I \subset A$ un ideale e $\varphi_I$ la proiezione all'anello quoziente modulo $I$, $\pi_I$ induce una Corrispondenza biunivoca tra gli ideali di $\faktor{A}{I}$ e 
...
\begin{remark}
    ...
    con $\ker f = m\ZZ \cap n\ZZ = [m,n]\ZZ$, da cui:
        \[ \Z{[m,n]} \varlonghookrightarrow \Z{m} \times \Z{n}
            \]
    avevamo visto che $f$ è surgettiva se e solo se $(m,n) = 1$, ed in questo modo $[m,n] = mn$ (cioè è vero 
    in questo caso che $IJ = I \cap J$), dunque:
        \[ \Z{[m,n]} = \Z{mn} \cong \Z{m} \times \Z{n}
            \]
    pertanto la nuova versione del teorema Cinese è una generalizzazione della precedente.
\end{remark}
...
\begin{remark}
    Spesso il \hyperref[zorn]{Lemma di Zorn} viene usato su famiglie $\mathcal{F}$ di ideali ordinati secondo la relazione di inclusione.
\end{remark}
...
\begin{proof}
    ...
    \begin{enumerate}
        \item Sia $I \subsetneq A$ un ideale e sia:
            ...
            \[ \mathscr{C} = \{J_n\}
                \]
            allora $\bigcup J_n\footnote{Andrebbe dimostato che l'unione di ideali in catena, analogamente a quanto accade per i sottogruppi, è un ideale.} = J \in \mathcal{F}$ con
            $J$ maggiorante di $\mathscr{C}$. Infatti:
            \begin{itemize}
                \item $\forall J_n \in \mathscr{C}$, ovvio per costruzione, avendo preso $J$ come l'unione di tutti i $J_n$.
                \item $J \in \mathcal{F}$, poiché $I \subset J_n \subset J$, $\forall J \in \mathcal{F}$ ed inoltre $J$ è un ideale proprio, infatti, se $1 \in J = \bigcup J_n \implies \exists n$ tale che
                    $1 \in J_n \subsetneq A$ che è assurdo.
            \end{itemize}
            Dunque ogni catena di $\mathcal{F}$ ammette maggiorante, pertanto $\mathcal{F}$ è induttivo e vale il \hyperref[zorn]{Lemma di Zorn}, per il quale la famiglia $\mathcal{F}$ ammette almeno un elemento massimale $M$.
            Resta da verificare che tale elemento massimale $M$ sia un ideale massimale (poiché la famiglia $\mathcal{F}$ non è la famiglia di tutti gli ideali di $A$) nella famiglia di tutti gli ideali propri di $A$, ma ciò segue subito osservando che,
            preso $L \subsetneq A$ ideale proprio con $M \subseteq L$, allora:
                \[ I \subseteq M \subseteq L \implies L \subset \mathcal{F}
                    \]
            e per la massimalità di $M$ in $\mathcal{F}$, segue che $M = L$.
        \item Segue immediatamente dal punto (1), infatti, si $x \in A \setminus A^*$, allora per la \hyperref[2.26]{Proposizione 2.26} l'ideale generato da $x$ è proprio,
...
\begin{proof}
    Verifichiamo le affermazioni:
    \begin{enumerate}[(1)]
        \item Presi $x,y \in A$, per definizione abbiamo che $I$ è primo se e solo se $xy \in I \implies x \in I \vee y \in I$, d'altra parte, $\faktor{A}{I}$ è un dominio se e solo se:
            \[ (x+I)(y+I) = I \implies x \in I \vee y \in I
                \]
            ovvero se e solo se $xy+I = I \implies x \in I \vee y \in I \footnote{Ovvero uno dei due elementi è nella classe laterale neutra dell'anello quoziente.} \iff xy \in I \implies x \in I \vee y \in I$, ma ciò è equivalente a dire che $I$ è primo.
        \item Per il \hyperref[corrispondenza]{Teorema di Corrispondenza} $I$ è un ideale massimale se e solo se in $\faktor{A}{I}$ ci sono soltanto gli ideali $(\ol 0)$ e $\faktor{A}{I}$ (ovvero se gli ideali che lo contengono sono soltanto sé stesso e tutto l'anello) 
        dunque per il (2) della \hyperref[2.26]{Proposizione 2.26} se e solo se $\faktor{A}{I}$ è un campo.
    \end{enumerate}
\end{proof}
...
\begin{corollary}
    ...
    \[ \pi_I : A \longrightarrow \faktor{A}{I}
        \]
    conserva ideali primi e massimali.
\end{corollary}
