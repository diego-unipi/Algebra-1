...
Abbiamo già osservato che è un'azione (ovvero che $\varphi$ è un omomorfismo). In questo caso:
    \[ \Orb(x) = \{\varphi_g(x) | g \in G\} = \{ gxg^{-1} | g \in G\} = \Cl_G(x)
        \]
dove $\Cl_G(x)$ prende il nome di \vocab{classe di coniugio} di $x$ (si indica anche con $C_x$ quando è chiaro il gruppo). Mentre:
...
\begin{definition}
    Dato $A$ un UFD e $f(x) \in A[x]$, $f(x)$ si dice \vocab{primitivo} se $c(f(x)) = 1$.
\end{definition}
...
\begin{corollary}
    \label{2.118}
    Siano $f(x),g(x) \in A[x]$, con $f$ primitivo e $f(x) \mid g(x)$ in $K[x]$, allora $f(x) \mid g(x)$ in $A[x]$.
\end{corollary}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{multline*}
        	...
        	\implies f \mid c(g(x)) \mid g(x) \quad \text o \quad f \mid c(h(x)) \mid h(x)
        \end{multline*}
...
\begin{remark}
    ...
    In $\ZZ[i]$ la fattorizzazione in irriducibili di $2$ è $2 = -i(1+i)^2$, quindi $p = 1+i$ (a meno di associati); 
    ma $1+i \mid a+bi \implies N(a+bi) = a^2+b^2$ è pari [in realtà vale "se e solo se" ma non ho voglia di scrivere l'altra freccia]
...
\begin{remark}
    ...
    \[ uv = 1 \implies 
       \footnote{Stiamo usando il fatto che nei complessi il prodotto fa 1 se è tra elementi coniugati. [Detto così è falso; il modo giusto di farlo è con la norma (vedi Esercitazioni)]
       } \quad 
...
\begin{proof}
    La dimostrazione è identica a quella già vista in 
    \href{https://github.com/diego-unipi/Appunti-Aritmetica}{\textcolor{purple}{Aritmetica}}. 
    Siano $[F:K] = n$ e $[L:F] = m$, verifichiamo che $[L:K]=nm$;
    per definizione sappiamo che $[F:K] = n$ ovvero $F$ è un $K$-spazio vettoriale con $\dim_KF = n$,
    e ugualmente, $[L:F] = m$ ovvero $L$ è un $F$-spazio vettoriale con $\dim_FL = m$. 
    Possiamo considerare allora una $K$-base di $F$, $\{v_1,\ldots,v_n\}$, ed una $F$-base di $L$, $\{w_1,\ldots,w_m\}$; 
    per dimostrare la tesi, dobbiamo dimostrare che $\dim_KL = nm$, ovvero $L$ ammette una $K$-base di cardinalità $nm$. 
    Consideriamo l'insieme $\{v_iw_j\}_{i=1,\ldots,n}^{j=1,\ldots,m}$, come si osserva facilmente, 
    esso ha cardinalità $nm$, dimostriamo quindi che tale insieme è una $K$-base di $L$, 
    per fare ciò verifichiamo separatamente che gli elementi di $\{v_iw_j\}_{i=1,\ldots,n}^{j=1,\ldots,m}$ 
    generano tutti gli elementi di $L$ e che sono tra loro linearmente indipendenti:
	\begin{itemize}
...
\begin{proposition}[Campo delle estensioni algebriche][Si chiama così? dovrebbe essere campo degli elementi algebrici]
    \label{3.12}
...
\begin{definition}
    Sia $f(x) \in K[x]$, con $\deg f(x) \geq 1$, e, detta $\overline K$ una chiusura algebrica di $K$, siano $\alpha_1,\ldots,\alpha_n \in \overline K$ le radici di $f(x)$, si definisce \vocab{campo di spezzamento} di $f(x)$ su $K$ il sottocampo di $\overline K$:
...
\newpage
[Forse sarebbe il caso di fare un nuovo capitolo qui]
...
\begin{definition}
    Un campo $K$ tale che tutti i polinomi irriducibili in $K[x]$ hanno derivata non nulla prende il nome di \vocab{campo perfetto}.
\end{definition}
...
\begin{definition}
    Data un'estensione algebrica $K \subset L$, essa si dice \vocab{separabile} se il polinomio minimo di ogni elemento di $L$ è un
    \vocab{polinomio separabile}, ovvero ha radici tutte distinte nel suo campo di spezzamento (o equivalentemente in una chiusura algebrica di $K$).
...
\begin{example}
    ...
    \footnote{Cioè \mu(x+1) = \frac{(x+1)^p - 1}{(x+1)-1}} = \frac{x^p}{x} = x^{p-1}}
    pertanto i coniugati di $\zeta_p$ sono $\zeta_p^i$, con $1 \leq i < p$ (ovvero sono $p-1$), da cui:
    \[ [\QQ(\zeta_p) : \QQ] = \phi(p) = p - 1 
        \]
    Le immersioni di $\faktor{\QQ(\zeta_p)}{\QQ}$ sono del tipo:
    \[ \varphi_i : \QQ(\zeta_p) \longrightarrow \overline \QQ : \zeta_p \longmapsto \zeta_p^i \qquad \text{con} \quad \varphi_{i|\QQ} = id_\QQ
        \]
...
\begin{definition}
    ...
    \[ \forall \varphi : F \varlonghookrightarrow \overline K \qquad \text{con} \quad \varphi_{|K} = id_K
        \]
...
\begin{example}
    ...
    \begin{itemize}
        ...
        \[ \varphi : F \varlonghookrightarrow \overline \QQ \qquad \text{con} \quad \varphi_{|\QQ} = id_\QQ
            \]
        abbiamo che:
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{multline*}
            \varphi(F) = \varphi(K(\{\alpha_{ij}\} \mid i = 1,\ldots,k, j = 1, \ldots, n_i)) = \\
...
\[ \left\{\varphi(\alpha) ~\Big|~ \varphi \in \Gal\left(\faktor{\QQ(\zeta_7)}{\QQ}\right) \right\}
    \]
...
\begin{proof}
    ...
    ovviamente $k = \ord \phi \mid d$; d'altra parte se $\phi^k = id$, $\forall \alpha \in \F{q^d}$ si ha che:
...
\begin{proof}
	...
	Sia $s = 2$ e $n = [E : K]$, allora abbiamo $E = K(\alpha,\beta)$ e per il \hyperref[3.39]{Corollario 3.39}:
        \[ \exists~ \varphi_1,\ldots,\varphi_n : E \varlonghookrightarrow \ol K \qquad \text{con} \quad \varphi_{i|K} = id_K
            \]
...
\begin{enumerate}[(1)]
    ...
    pertanto $\alpha$ è fissato da tutti gli elementi di $\left<H,S\right>$, da cui
