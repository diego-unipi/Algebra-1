...
\begin{proof}
    Osserviamo che:
    \[ \forall \varphi : \F{q^d} \varlonghookrightarrow \overline \Fp \qquad \text{con $\varphi_{|\F{q^d}} = id_{\F{q^d}}$}
        \]
...
\begin{proof}
    ...
    Verifichiamo ora la seconda parte del teorema; sappiamo che $H \triangleleft \Gal\left(\faktor{L}{K}\right) \iff gHg^{-1} = H$, $\forall g \in \Gal\left(\faktor{L}{K}\right)$ e per il 
    \hyperref[3.67]{Lemma 3.67} abbiamo che la condizione posta dall'equazione è equivalente a quella di prima:
    \[ g(L^H) = L^{g H g^{-1}} = L^H \qquad \forall g \in \Gal\left(\faktor{L}{K}\right)
        \]
